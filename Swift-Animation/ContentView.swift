...
//  swiftanimation
//
//  Created by Hamal Saefudin on 06/12/22.
//
...
struct ContentView: View {
    ...
    var body: some View {
        ScrollView {
            ...
            PercentageView()
            DotLoadingView()
            //Assignment 6, marks: recording button view
            RecordingButtonView()
...
//MARK: LOADING PERCENTAGE VIEW
struct PercentageView: View {
    @State private var labelPercentage : CGFloat = 0;
    @State private var isLoadingProgress : Bool = false;
    var body: some View {
        ZStack {
            Text("% \(Int(labelPercentage*100))")
            Circle()
                .stroke(Color(.systemGray5),lineWidth: 14)
                .frame(width: 150,height: 150)
            Circle()
                .trim(from: 0, to: labelPercentage )
                .stroke(Color.purple,lineWidth: 7)
                .frame(width: 100,height: 100)
        ...
        .onAppear{
            Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { timer in
                labelPercentage += 0.05
                if labelPercentage >= 1.0{
                    timer.invalidate()
                }
            }
        }
        ...
        VStack{
            ZStack{
                ...
                RoundedRectangle(cornerRadius: isStop ? 35 : 10)
                    ...
                    .foregroundColor(isStop ? .blue : .purple)
            }.onTapGesture {
